...
26. The admin seed UI and navigation MUST be restricted to the currently connected owner of unexpired ncrmro.eth on Sepolia. The seed MUST recheck wallet identity and network before every new transaction, preserve populated or incompatible ENS links, and support wallet-signed checkpoint/resume. Only verified-empty admin-owned legacy registries MAY be relinked, with previous addresses checkpointed. The browser seed MUST use separate admin-controlled demo participant contracts and publish its verified discovery index on-chain for both public websites. Newly registered names MUST be distinguished from populated artwork/gallery registries.

27. Admin seeding MUST discover available participant names, include newly acquired names on subsequent runs, and avoid duplicate deployments or records. Public-domain image fixtures MUST retain source attribution and valid content hashes. Bundled images MUST NOT be represented as publicly pinned unless pinning succeeds.
