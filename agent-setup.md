...
Work in the feature worktree, preserve the default checkout, and keep the old ETH Tokyo project untouched. This session authorizes implementation, Cloudflare deployment, and a persistent local Tailscale server. The earlier documentation-only limit is superseded.
...
Use `npm run dev` for port allocation and Tailscale binding. The running session uses user service `eonmoon-beta-local`; inspect it before starting another instance. Stop with `systemctl --user stop eonmoon-beta-local` when requested.
