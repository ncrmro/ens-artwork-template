...
Public browsing is separate from management: `/browse/art/`, `/browse/galleries/`, and `/browse/exhibitions/` read ENS registries without connecting a wallet or selecting a tenant. The local chain supplies its seeded names; public Sepolia browsing lets visitors look up a name rather than pretending to provide a global index. The header namespace selector switches between `art.<name>` artist management and `exhibitions.<name>` gallery management for names the connected wallet can access. `/workspace/` resumes the last management mode.
