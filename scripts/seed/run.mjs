...
import { createAssets, contenthash } from "./assets.mjs";
...
export async function seedCatalogue({
  ...
  for (const p of participants) {
    if (!/^[a-z0-9]+(?:-[a-z0-9]+)*\.eth$/.test(p.parent))
    ...
    try {
      ...
      if (!(await rootRead("hasRoles", [resource, 1n << 20n, p.wallet])))
        issues.push(p.id + ": wallet lacks ENS subregistry permission");
      if ((await pc.getBalance({ address: p.wallet })) === 0n)
  ...
  const signer = createPublicClient({ chain, transport: http(signerUrl) });
  ...
  for (const p of participants)
    if (!accounts.some((a) => eq(a, p.wallet)))
  ...
  if (fs.existsSync(plan.journal)) {
    journal = JSON.parse(fs.readFileSync(plan.journal));
    if (journal.fingerprint !== fingerprint)
      throw Error(
        "Seed plan, chain, data or contract artifacts changed. Use a new journal; do not overwrite existing namespaces.",
      );
  }
  fs.mkdirSync(plan.journal.slice(0, plan.journal.lastIndexOf("/")) || ".", {
    recursive: true,
  });
  const persist = () => {
    fs.writeFileSync(plan.journal + ".tmp", JSON.stringify(journal, null, 2));
  ...
  const assets = await createAssets(catalogue);
  ...
  for (const [i, h] of catalogue.history.entries()) {
  ...
  fs.writeFileSync(plan.indexOutput, JSON.stringify(index, null, 2) + "\n");
  return {
    chainId: plan.chainId,
    index,
